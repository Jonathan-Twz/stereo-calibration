...
def calibrate_single(file_dir):
    ...
    pattensize = 0.06 # (m)
    real_cor = np.zeros((7 * 6, 3), np.float32)
    real_cor[:, :2] = np.mgrid[0:7, 0:6].T.reshape(-1, 2) * pattensize
    ...
    for pic in pic_name:
        ...
        # 寻找到棋盘角点
        succ, pic_cor = cv.findChessboardCorners(pic_data, (7, 6), None)
    ...
    _, cameraMatrix, distCoeffs, _, _ = cv.calibrateCamera(real_points, pic_points, (480, 752), None, None)
...
if __name__ == '__main__':
    ...
    #测试图片
    org = cv.imread(r'../pic/left/871.png')
    org2 = cv.imread(r'../pic/right/871.png')

    #显示线条，方便比较
    dst = cv.remap(org, left_map1, leftmap2, cv.INTER_LINEAR)
    for i in range(20):
        cv.line(dst, (0, i*24), (752, i*24), (0,255,0), 1)

    dst2 = cv.remap(org2, right_map1, right_map2, cv.INTER_LINEAR)
    for i in range(20):
        cv.line(dst2, (0, i*24), (752, i*24), (0,255,0), 1)

    canvas = np.zeros((480,752*2,3), dtype="uint8")
    canvas[:, :752] = dst
    canvas[:, 752:] = dst2
    cv.line(canvas, (752, 0), (752, 480), (255,0,0), 1)
